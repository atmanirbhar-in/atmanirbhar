defmodule Atmanirbhar.Catalog.TaxonomyImporter do
  ...
  def import do
    ...
    |> Stream.map(fn set_of_records ->
      Enum.zip(
        [:uniq, :cat0, :cat1, :cat2, :cat3, :cat4, :cat5, :cat6],
        set_of_records
      )
      |> Enum.into(%{})
      |> process
  ...
  def create_record(uniq, name, fullname, parent_name) do
    ...
    parent_uniq =
      case parent_taxonomy do
        nil -> nil
        _ -> Map.get(parent_taxonomy, :uniq)
      end

    changes = %{uniq: uniq, name: name, full_name: fullname, parent_uniq: parent_uniq}

    result =
      case Repo.get_by(Taxonomy, uniq: uniq) do
        # not found, we build one
        nil -> %Taxonomy{}
        # exists, let's use it
        taxonomy -> taxonomy
      end
    ...
    case result do
      ...
      {:ok, _struct} -> IO.puts("created")
      # Something went wrong
      {:error, _changeset} -> IO.puts("failed")
    end
  end
end
